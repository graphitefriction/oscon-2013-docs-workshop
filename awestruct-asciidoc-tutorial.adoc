...
.Create a partial, +_partials/post-content.html.haml+ to layout the post
[source,haml]
----
%article.post
  %header.post-head
...
layout: base
---
=partial 'post-content.html.haml', :post => page
----
...
.row
  .large-12.columns.posts
    - page.posts.each do |post|
      =partial 'post-content.html.haml', :post => post, :index => true
...
.Add tag links to bottom of +_partials/post-content.html.haml+
----
  %footer.post-footer
    - if page.post.tags
      .tags
...
.Edit +_partials/post-content.html.haml+ to add Disqus comments
[source,haml]
----
  %hr
  - if !page.listing
